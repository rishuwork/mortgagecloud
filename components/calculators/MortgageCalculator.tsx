...
export const MortgageCalculator = () => {
  ...
  const calculations = useMemo(() => {
    ...
    let totalInterest = 0;
    let totalPaid = 0;
    let actualPayoffYears = inputs.amortization;
    ...
    if (inputs.paymentFrequency === "monthly") {
      ...
      // Regular bi-weekly: same annual payment as monthly, just split differently
      displayPayment = (monthlyPayment * 12) / 26;
      totalPaid = monthlyPayment * totalPayments;
      totalInterest = totalPaid - totalPrincipal;
    } else if (inputs.paymentFrequency === "accelerated") {
      // Accelerated bi-weekly: half monthly payment, 26 times/year
      // This results in 1 extra monthly payment per year
      displayPayment = monthlyPayment / 2;

      // Simulate actual payoff with accelerated payments
      let balance = totalPrincipal;
      let totalInterestPaid = 0;
      let payments = 0;
      const maxPayments = inputs.amortization * 26 * 2; // Safety limit

      while (balance > 0 && payments < maxPayments) {
        const interestPayment = balance * biweeklyRate;
        const principalPayment = Math.min(displayPayment - interestPayment, balance);
    ...
    return {
      ...
      cmhcPremium,
      downPaymentPercent,
      payoffYears: actualPayoffYears,
      yearsSaved: inputs.paymentFrequency === "accelerated"
        ? inputs.amortization - actualPayoffYears
        : 0,
    };
  }, [inputs]);
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="mt-8 p-6 rounded-xl bg-secondary border border-border">
          <div className="text-center mb-6">
            ...
            {calculations.yearsSaved > 0 && (
              <p className="text-sm text-success mt-2 font-medium">
                🎉 Pays off {calculations.yearsSaved.toFixed(1)} years faster!
              </p>
            )}
          </div>
